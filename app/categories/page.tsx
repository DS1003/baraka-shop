...
export default function CategoriesPage() {
    return (
        <main className="bg-[#f8f9fb] min-h-screen pb-24">
            {/* Simple Header */}
            <div className="bg-[#1B1F3B] py-20 mb-20 relative overflow-hidden">
                <div className="absolute top-0 right-0 w-1/2 h-full bg-primary/20 blur-[150px] rounded-full translate-x-1/2" />
                <Container className="relative z-10">
                    <h1 className="text-4xl md:text-6xl font-black text-white uppercase tracking-tighter mb-4">Toutes nos catégories</h1>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-10">
                    {ALL_CATEGORIES.map((cat, idx) => (
                        ...
                        >
                            ...
                            <div className="p-10">
                                <h3 className="text-2xl font-black text-[#1B1F3B] uppercase tracking-tighter mb-6">{cat.name}</h3>
                                <div className="flex flex-wrap gap-2 mb-8">
                                    {cat.sub.map((s) => (
                                        <span key={s} className="px-4 py-2 bg-gray-50 text-gray-400 text-[10px] font-bold uppercase rounded-xl hover:bg-primary/10 hover:text-primary transition-all cursor-default">
                                            {s}
                                        </span>
                                    ))}
                                </div>
                                <Link href={`/category/${cat.slug}`} className="w-full h-14 bg-[#1B1F3B] text-white rounded-2xl font-black text-[10px] uppercase tracking-widest flex items-center justify-center gap-3 hover:bg-primary transition-all shadow-xl hover:shadow-primary/20 group-hover:gap-5">
                                    Explorer le rayon <ArrowRight className="w-4 h-4 text-primary" />
                                </Link>
                            </div>
                        </motion.div>
                    ))}
